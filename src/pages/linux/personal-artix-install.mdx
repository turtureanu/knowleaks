...
We need to temporarily delete the /.snapshots directory, so we ca recreate it using `snapper`:
...
You may also use a tool such as `KeyTool.efi`, if your UEFI doensn't support manually loading keys from the interface.
